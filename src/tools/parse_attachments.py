...
PARSE_ATTACHMENTS_SCHEMA = {
    ...
    "input_schema": {
        ...
        "properties": {
            ...
            "filename": {
                "type": "string",
                "description": "Specific file, e.g. 'report.pdf'. Omit to parse all PDF/image files.",
            },
        },
        "required": ["claim_id"],
    },
...
def parse_attachments(claim_id: str, filename: str | None = None) -> dict:
    ...
    files = (
        [claim_dir / filename]
        if filename
        else [f for f in claim_dir.iterdir() if f.suffix.lower() in all_exts]
    )
    if not files:
    ...
    for f in files:
        ...
        if f.suffix.lower() == ".pdf":
            if not _PYMUPDF:
                return {"isError": True, "code": "PARSE_FAILED",
                        "guidance": "PyMuPDF not installed — run: pip install pymupdf"}
            try:
                doc = fitz.open(str(f))
                texts.append("\n".join(page.get_text() for page in doc))
            except Exception as exc:
                return {"isError": True, "code": "PARSE_FAILED",
                        "guidance": f"Failed to parse {f.name}: {exc}"}
    return {
        "claim_id": claim_id,
        "attachments": [f.name for f in files if f.exists()],
        "text": "\n\n".join(texts),
    }
